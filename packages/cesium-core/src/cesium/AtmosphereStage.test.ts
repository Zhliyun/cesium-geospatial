...
describe('validateAtmosphereOptions', () => {
  it('默认值（B 路径全量 + 动态曝光 + lensflare 默认 + depthTemporal temporal*）', () => {
    expect(validateAtmosphereOptions({})).toEqual({
      ...
      moon: true,
      moonRadianceScale: 774, // 160×2.2²——ω 拍板 0.0099 后同式保显示亮度（2026-09-01）
      moonAngularRadius: 0.0099, // 物理×2.2（2026-09-01「过小放大」：沿革 0.0135→0.03→0.02→0.0045→0.0099）
...
describe('月光 options/state/uniforms（spec r2 §5.4）', () => {
  it('validate 默认：moonRadianceScale=774 / moonAngularRadius≈0.0099（moon=true 已固化于默认值用例）', () => {
    const r = validateAtmosphereOptions({})
    expect(r.moon).toBe(true)
    expect(r.moonRadianceScale).toBe(774)
    expect(r.moonAngularRadius).toBeCloseTo(0.0099, 8)
  ...
  it('buildAtmosphereUniforms 含月盘三件：moonDirection 闭包 + 静态 ω/radiance', () => {
    ...
    expect(u.moonAngularRadius).toBeCloseTo(0.0099, 8)
    expect(u.u_moonRadiance).toBe(774)
