...
describe('月光 options/state/uniforms（spec r2 §5.4）', () => {
  it('validate 默认：moonRadianceScale=1546 / moonAngularRadius≈0.014（moon=true 已固化于默认值用例）', () => {
    const r = validateAtmosphereOptions({})
    expect(r.moon).toBe(true)
    expect(r.moonRadianceScale).toBe(1546)
    expect(r.moonAngularRadius).toBeCloseTo(0.014, 8)
  ...
  it('buildAtmosphereUniforms 含月盘三件：moonDirection 闭包 + 静态 ω/radiance', () => {
    ...
    expect(u.moonAngularRadius).toBeCloseTo(0.014, 8)
    expect(u.u_moonRadiance).toBe(1546)
