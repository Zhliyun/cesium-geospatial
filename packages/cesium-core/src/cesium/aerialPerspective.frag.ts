...
${o.moon ? '    moonDisc *= limbFade; // 与太阳盘行为一致（太空视角 limb 渐隐）\n' : ''}  }
  ...
  // 物理上太阳 <-18°（天文暮光末）天空太阳散射应为零。窗口 sin(-18°)→sin(-4°)：白天/民用暮光
  // （>-4°）零回归，航海→天文暮光渐消，深夜全消（晨昏线附近的暮光红完整保留在此窗口上方）。
  // muSunSky 用相机径向法线（与 clouds.frag nightFactor 同源几何——相机当地太阳仰角）。
  float muSunSky = dot(normalize(cameraPosition), sunDirection);
  float skyNightFade = 1.0 - smoothstep(-0.309, -0.0698, muSunSky);
