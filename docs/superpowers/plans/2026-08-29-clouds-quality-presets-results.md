...
| 帧率台阶 | **通过**（dpr2 组完整单调） | 5120×2880：帧时间 16.70 ≤ 16.79 ≤ 18.11 ≤ 19.80 ms；ultra 跌破 vsync（42-50 FPS） |
...
**三级分辨率递进**（每档独立浏览器会话、截图读 `debugShowFramesPerSecond` overlay，FPS 读数经裁剪放大交叉核验）：

| 分辨率 | low | medium | high | ultra | 判定 |
|---|---|---|---|---|---|
| 1280×720（默认视口） | 16.94 / 59 | 16.80 / 59 | 17.69 / 59 | 18.49 / 59~64* | 四档锁 vsync，无区分度（M 系列 + 小视口 GPU 余量过大）→ 按 spec 放大负载重测 |
| 2560×1440 | 16.74 / 59 | 16.7x / 59 | 16.65 / 60 | **23.64 / 42** | ultra 跌破 vsync；low/medium/high 仍锁帧（三者无区分度） |
| 5120×2880（viewport 2560×1440 × dpr2） | 16.70 / 60 | 16.79 / 59 | 18.11 / 55 | **19.80 / 50** | **完整单调台阶：16.70 ≤ 16.79 ≤ 18.11 ≤ 19.80 ms**（FPS 60 ≥ 59 ≥ 55 ≥ 50） |

\* 720p ultra 单次读数 FPS 与帧时间自相矛盾（18.49ms ≈ 54fps vs 显示 64），以帧时间为准，标注为锁帧噪声带边缘。

**判定：通过**。dpr2 组（5120×2880，四档同批连续录制、条件一致可比性最强）呈现完整单调帧时间台阶且 ultra 跌破 vsync 上限，满足「帧时间 low ≤ medium ≤ high ≤ ultra + 至少一档跌破」判据。2560×1440 组进一步佐证 ultra 独先跌破。

截图：`fps-2560-{low,medium,high,ultra}-*.png`、`fps-dpr2-{low,medium,high,ultra}-*.png`（读数裁剪放大件 `crop-*.png`）。

**偏差记录（如实）**：
1. 2560 组 ultra 单次读数 23.64ms 高于 dpr2 组 ultra 19.80ms（分辨率更低反而帧时间更高）——两组非同批录制，单帧 overlay 读数含瞬时波动/机器热状态差异；以同批 dpr2 组为台阶主证据，2560 组仅取「ultra 跌破」定性结论。
2. 「换档瞬间 shader 编译长帧」旁证：720p 组 high 17.69 / ultra 18.49 帧时间超出锁帧噪声带（16.7-17.0），为弱旁证（录制窗口内含切档后的偶发超帧）；未做逐帧录屏确认。
...
**四项验收判据全部通过**（帧率台阶与热切换含上述偏差注记）；spec §11 三项退路全部未触发，档位表与测试期望无需修订。质量档位特性（Task 1-6，`2b22a17..d636b6f`）端到端验收收口。
