...
2. **ultra 独跌 vsync，两组分辨率一致**：19.80 ms / 50 FPS @5120×2880 与 23.64 ms / 42 FPS @2560×1440——档位真实生效的直接证据。ultra 相对 high 的负载差异仅两处（qualityPresets.ts）：minStepSize 50→10（主 march 采样密度 5×）+ BSM mapSize 512→1024（4× 像素），两者叠加独占性地超出 60 FPS 帧预算。
3. dpr2 组 high 档一张截图捕得 **46.67 ms 单帧长帧**（同会话另一张 16.52 正常）——该截图来自独立浏览器会话直开、非换档瞬间，长帧成因未知（如实记录，不作归因）；换档真实切换的证据以 Step 4 的 1000 条 setQuality console 日志为准。
