...
- 轨道反超的候选归因（假说）：rotateLeft 只改 heading、相机位置不动 → world 矩阵理论上完全冻结（增量应≈静止 0）→ 但实测 1.5%——说明轨道增量主体并非矩阵通道，而是「视差扫过 × BSM 高频云影纹理」的固有画面变化（开/关两组云影纹理频率不同，差分不抵消）；frustum 同样携带该分量，两模式差异被组间方差（±0.3%）淹没。
...
| 轨道 1e-5 | 1.02（含视差×BSM 分量，见 §1） | 0.72（同） |

**处置建议（spec §7 首要风险行路径）**：判据豁免/放宽（非 snap 地板 0.24-0.48% 结构性存在，运动工况「增量」还含视差×BSM 固有分量，0.3% 对该指标定义过紧），或立项 STBN 世界域索引 + 边界处理（大改）；snap 残余专项候选=「跳变帧（每 ~5 帧跨 c0 texel 62.5m）重 march + jitter 相位重随机化被静止跳过折叠成 ~12Hz 脉冲」。
...
| §6 E1' 归因实验 | ✅ 已做（两轮）：地板 0.24~0.48%，可能自身超判据 |
...
**总判定：验收执行完成、数据齐全（干净管线复测）；四探针目标不达标（DONE_WITH_CONCERNS）**。世界锚定静止完美、平移/dolly 工况矩阵通道削减 14-34%、轨道反超（视差×BSM 固有分量主导）。生产代码未动（按 brief 约束）。后续动作待裁决：判据豁免/重定义（分离「视差×BSM 固有分量」与「闪动分量」），或 STBN 世界域索引/边界处理立项 + 分档 radii（低太阳角近景软化）。
