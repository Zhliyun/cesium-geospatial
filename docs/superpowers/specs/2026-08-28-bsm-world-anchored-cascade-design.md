...
- **close zoom 精度回退（已知项，评审 render-3a）**：现 far≈20km 时 cascade0 texel 15.6m（视锥收紧红利），固定 radii 后统一 47m——近景 texel 粗 3 倍，PCF 世界宽度同步变粗。低太阳角近景专看（§6）；过软则按相机高度分 2-3 档 radii（高度量化仍时序稳定，档间一次跳变可 EMA）。
