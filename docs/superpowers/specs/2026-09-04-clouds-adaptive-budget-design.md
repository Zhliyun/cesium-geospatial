# 云预算自适应设计——贴地掠射+黄昏性能（A 太阳角影子预算 / B 视线仰角段长收紧）r3

- 日期：2026-09-04（r3：Phase 0 实测回填+B 弃案终审定稿；r2 三专家评审修订、r1 见 git 历史）
- 状态：已定稿（三专家评审+终审通过，Phase 0 实测回填；裁决全部采纳，见 §9 评审记录）
...
钉 10:00Z 太阳当地仰角 **13.288°**（守卫同源实算定稿值；红队手算 11.3° 偏低 ~2°，
终审 NOAA 独立公式 13.491° 佐证）。
...
3. 仰角-成本扫描（静止 p50）：04:00Z（太阳~68°）33.3 / 09:30Z（~20°）29.1 /
   10:00Z（13.288°）31.5 / 10:30Z（~6°）31.1ms——**静止成本对太阳角不敏感**
   （BSM 静止帧跳过重画=既有机制实证）。
...
下界由此推导且 **≥0.5 硬性**；不满足则弃案回用户（不自动加深收缩）。现状撞线域
已由 Phase 0 补算（见下）。

Phase 0 补算（终审核实）：FLOOR=0.5 → maxIter=25×maxStep=1000=25km 覆盖，撞线仰角
≈3.9°（现状 maxIter=50 撞线 1.95°）——A 新增截断带仅 (1.95°, 3.9°)，plan 收窄判据
（>5°）不触发；黄昏 <4° 影子尾部 banding 列入真机目验观察项。
...
> 收紧省成本」前提被证伪，主 march 采样数不是该视角瓶颈。按 §2 实测结果的预置判据
> 「无斜率则 B 弃案」执行。以下原设计文本保留作历史记录（若未来视角类变化可复活，须重探针）。
...
若开 shadowTemporal=1 须等 resolve 收敛再截）；②黄昏影子/地平线云带目验（A 截断
带 <4° 影子尾部观察项+画质目验）；③日本 500m 云海机位回归；④傍晚实时时钟复测原场景。突刺协议钉死：测量
...
- 域外激活必须可一键回退（A 逃生门）且回退逐位（uniform 同值论证+真机双 URL
  截图 diff=0；r3 注：四组成对验收数据合并前由控制器补测，画质目验由用户执行）。
