...
# 终测轮 4（0b335ab 双源修复 + 5b7a382 SkyStage 死帧修复后，2026-09-03，最终轮）

**统一口径**：全部 URL 显式 `mode=atmosphere`；每组 tilesLoaded 确认；**新增 renderAlive 探针**（rAF 200ms 两次推进验证，防 5b7a382 前的 render loop 停摆死帧伪像）——本轮全部样本 renderAlive=true。同参数跨加载确定性顺带复验：r4-sanity vs r4-b5-baked **逐位 0**（B4 在最终代码上成立）。

## 十项判定一行式

| # | 项 | 判定 | 核心数据 |
|---|---|---|---|
| 1 | B5 白雾终判 | **PASS** | 云单体结构恢复（`r4-b5-baked.png`：近景积云群+蓝隙+中景云甲）；rows mean 1.36→**2.34**（+72%）；baked vs escape diff **59.4%**（meanAbs 10.83）；escape 侧 rows 与历史逐位同=旧行为未破坏 |
| 2 | B8 预设单调性 | **PASS**（caveat 一条） | 中景带（y45-75%）：clear **0.597** < fair **0.880** < cloudy **1.000** ≤ overcast **1.000** 单调恢复；整图 clear vs overcast diff 62.4% 证明预设生效。⚠️ cloudy/overcast 在 8km 视角中景带双双饱和不可分——档间区分留给用户目验；顶部 35% 天空带四档读数逐位同 0.3020=远景带对预设不敏感的统计伪像（B8 类判定须用中景带或整图 diff） |
| 3 | seed A/B | **PASS** | 实机 seed 1 vs 1337：**58.4%** 去相关（与 T6 端到端读数一致）；「seed 链断」**改判撤销**（真因=SkyStage 编译炸死帧，5b7a382） |
| 4 | B1 演化 | **PASS** | play=1 Δ5min **36.0%**、Δ30min **41.7%**（随时间增长；雾期 0.045% 伪像改判） |
| 5 | B2 归因 | **PASS** | phase 0 vs 1800：云变 **53.1%** + 天顶亮度差 **0.79%**<2%（太阳钉死）——演化分量真实上屏 |
| 6 | B3 回绕 | **记录（设计确认项维持）** | 19070 vs 0 = **56.9%**——上轮 0.019%「闭合良好」系死帧+雾双伪像，改判：回绕差异由 wind 位差（19070s×8m/s=152.6km=1.523 tile 非整数→52km 全场错位）主导，与第一轮 36.9% 同族。整场无缝循环需 `windMps×evolutionHours ≡ tileKm` 整数（用户拍板） |
| 7 | windOffset | **PASS（疑点撤销）** | phase 0 vs 9000（72km 位移）= **55.3%**——平流端到端工作；上轮 0.13% 系死帧伪像改判 |
| 8 | repeat 联动 | **PASS** | 20 vs 100：diff **19.5%**、rows 2.62 vs 2.34——f0926b0 联动真实生效（上轮 1.48% 系死帧+雾双伪像） |
| 9 | 帧率 | **PASS（记录）** | baked **33.29** vs escape **39.30** fps——雾期 60/45 早退反证消失并反转；baked 结构真实化的成本 ≈ -15%（march 跑满+BSM 全量 vs 旧图简单内容），绝对值可接受、无异常 |
| 10 | 气候带三视角 | **PASS（快速复核）** | lat0 **0.186** / lat28 **0.0007** / lat50 **0.889**——ITCZ/副热带谷/风暴带三带显著分化，挖除语义回归未破坏带结构。⚠️ lat0 低云较旧图同视角（0.439）变薄 ~58%——挖除语义 L0 削减的副作用观察，观感拍板留给用户 |

## 前轮死帧伪像改判记录（5b7a382 前的实验受 render loop 停摆影响）

| 前轮结论 | 改判 |
|---|---|
| 复测轮「phase 0 vs 9000 = 0.14%、wind uniform 疑失效」 | **撤销**——死帧伪像；真机 55.3% 平流正常 |
| 复测轮「cloudsSeed 链断实锤（实机 0.086% 逐斑同图）」 | **改判**——独立读回「烘焙数学正常（99.6% 去相关）」结论仍成立且正确；实机侧死帧伪像，修复后 58.4% 端到端工作。「JS 传递链断」结论错误，撤 |
| 复测轮「B3 闭合良好 0.019%」 | **改判**——死帧+雾双伪像；真实回绕行为=wind 位差主导 56.9%，设计确认项维持 |
| 终测轮「seed 同布局 → JS 链断实锤」 | 同上撤销（该实验在死帧上） |
| 终测轮「白雾=雾底支配一切弱响应」 | **部分维持**：雾罩（L2 40.7% 饱和+L3 恒 1）确曾压制 B1/B8/phase 可见性；但 seed/wind 的「逐位同图」主因是死帧（雾只会稀释不会归零 diff） |
| 复测轮「B8 三档逐位同 0.3020 + overcast 反低」 | **改判**——死帧期预设重放不可见所致；本轮中景带单调正常 |
| 终测轮独立 WebGL2 读回（B 40.7% 饱和/A 恒 1/旧 png A 有分布/R-G 健康） | **维持有效**——独立环境不经过 render loop，直驱烘焙 shader；0b335ab 的修复正以此为据 |
| 烘焙耗时亚百毫秒、escape 行为复原、气候带第一轮结构 PASS | 维持 |

**方法论铁律（T8 沿革沉淀）**：云渲染验收四重门=显式 `mode=atmosphere` + tilesLoaded + renderAlive（rAF 推进）+ 同批成对；「逐位同图」必须先排除死帧伪像再当确定性证据。

## 最终意外发现

1. **lat0 低云变薄**：ITCZ 视角 cloudFrac 0.186 vs 旧图同视角 0.439——挖除语义（low=saturate(low-mid)）显著削减 L0 低云甲。气候带「结构」PASS 但 ITCZ 带绝对云量偏低，是否调 band 增益/挖除幅度属用户观感拍板。
2. **顶部天空带统计盲区**：pitch -8 机位下画面上 35% 是天空+远景带，对预设/纹理源变化均不敏感（四档读数逐位同 0.3020）——量化脚本默认带不适合预设类判定，已改用中景带（band 0.45,0.75）。
3. **B3 的 wind 不回绕**在真实渲染下显形为 56.9% 跳变——若产品要求「演化环=整场循环」，一行修法是让 windMps×evolutionHours≡tileKm（如 wind=8、period 5.3h、tile 100.19km → windMps≈7.87 即可闭合），拍板留用户。
4. 全链路终态：0b335ab 双源修复后 baked 端到端全部十项无 FAIL；剩余用户拍板三项=B3 整场循环语义、cloudy vs overcast 档间区分观感、ITCZ 低云量观感。
