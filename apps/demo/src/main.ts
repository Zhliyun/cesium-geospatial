...
async function main(): Promise<void> {
  ...
  if (mode === 'atmosphere') {
    ...
    // 纹理在载时 moonRadiance 默认基准（774=160×2.2²，ω 拍板 0.0099 时同式显示亮度）
    const moonRadianceDefault =
      moonSurfaceTexture != null ? 774 / moonSurfaceAlbedoMean : 774
    ...
    const options: AtmosphereStageOptions = {
      ...
      // ?moonRadiance= 月盘倍率（默认 774=160×2.2²，ω 拍板 0.0099 后同式保显示亮度，纹理在载时按均值放大）；
      // ?moonAngularRadius= 月盘角半径 rad（默认 0.0099=物理×2.2，2026-09-01「恢复正常大小」定 0.0045 后「过小放大 2.2 倍」：
      ...
      ...(getNumber('moonAngularRadius') != null
        ? (() => {
            ...
            const k = omega / 0.0099 // 库默认比（0.0099=物理×2.2，2026-09-01「过小放大 2.2 倍」）
            ...
            // moonRadianceDefault = 库默认 774（纹理在载时已按均值放大；AtmosphereStage.ts/demo 拍板基准）。
