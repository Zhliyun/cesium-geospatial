...
async function main(): Promise<void> {
  ...
  if (mode === 'atmosphere') {
    ...
    // 纹理在载时 moonRadiance 默认基准（7111=库默认 1440×(0.03/0.0135)²，ω 拍板 0.03 时同式显示亮度）
    const moonRadianceDefault =
      moonSurfaceTexture != null ? 7111 / moonSurfaceAlbedoMean : 7111
    ...
    const options: AtmosphereStageOptions = {
      ...
      // ?moonRadiance= 月盘倍率（默认 7111=旧 1440×(0.03/0.0135)²，ω 拍板 0.03 后同式保显示亮度，纹理在载时按均值放大）；
      // ?moonAngularRadius= 月盘角半径 rad（默认 0.03=物理×6.7，2026-08-31 拍板：环形山可见需 ≥65px 盘径；
      ...
      ...(getNumber('moonAngularRadius') != null
        ? (() => {
            ...
            const k = omega / 0.03 // 库默认比（0.03=物理 0.0045×6.7，2026-08-31 拍板）
            ...
            // moonRadianceDefault = 库默认 7111（纹理在载时已按均值放大；AtmosphereStage.ts/demo 拍板基准）。
